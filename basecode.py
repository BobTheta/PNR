...
def readOneBlock2(pstr,posid,fp):
	data = {}
	preKey = pstr[:posid]
	data[preKey] = pstr.strip()[posid+2:]
	while 1:
		p,pos = breadline(fp)
		if pos == -2: break
		elif pos==-1 or pos>20:
...
'''
def writedata2json2(fout,data,infobox):
...
def writedata2json(fout,data,infobox):
	fw = codecs.open(fout,'a',"utf-8")
	for key in data:
	    if key in ['Infoboxes', u'角色']: continue
	    data[key] = getEscapeTxt(data[key])
	if len(infobox)>0:
            for key in infobox:
                infobox[key] =  getEscapeTxt(infobox[key])
	data['Infoboxes'] = infobox	
